...
print "Best k,Best branch,Best flank,Recall,Precision,TwoPaco RT,TwoPaCo memory,L-Sibelia RT,L-Sibelia memory,spoa RT,spoa memory"
...
for experiment in ["1"]:
	...
	k_value.sort()
	param_file = open("param" + experiment + ".txt", "w")
	...
	for now_k_value in k_value:
		...
		p = join(mypath, str(now_k_value))
			...
			for f2 in listdir(p):
				...
				rt_memory = []
				for line in open(os.path.join(p2, "rt1.txt")):
					line = line.strip().split()
					rt_memory.append(line[1])
			...
			print >> param_file, ""
	print ','.join(str(x) for x in [best_k,best_global_branch,best_global_flank,best_global_recall,best_global_precision] + best_rt_memory)
